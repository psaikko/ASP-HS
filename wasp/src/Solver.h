...
Solver::minimizeUnsatCore(
    vector< Literal >& assumptions )
{
    cout << "c minimizeUnsatCore called" << endl;
    ...
    if( unsatCore->size() <= 2 )
    {
        ...
        setMaxNumberOfRestarts( originalMaxNumberOfRestarts );
        setMaxNumberOfSeconds( originalMaxNumberOfSeconds );
        cout << "c minimizeUnsatCore size <= 2" << endl;
        return;
    }

    Clause* minUnsatCore = unsatCore;

    vector< Literal > testLiterals;
    vector< Literal > criticalLiterals;

    cout << "init testLiterals" << endl;
    for (unsigned i = 0; i < unsatCore->size(); ++i) {
        Literal lit = unsatCore->getAt( i );
        testLiterals.push_back( lit );
    }

    unsatCore = NULL;

    cout << "begin loop" << endl;
    while (testLiterals.size()) {
        clearAfterSolveUnderAssumptions( assumptions );
        clearConflictStatus();
        assumptions.clear();

        cout << "choose test lit" << endl;
        Literal test = testLiterals.back();
        testLiterals.pop_back();


        cout << "set assumptions" << endl;
        for (Literal l : testLiterals) {
            assumptions.push_back( l );
            setAssumption( l, true);
        }

        for (Literal l : criticalLiterals) {
            assumptions.push_back( l );
            setAssumption( l, true);
        }
        ...
        if ( result == INCOHERENT ) {
            cout << "redundant " << test.getId() << endl;
            cout << minUnsatCore->size() << " -> " << unsatCore->size() << endl;
            delete minUnsatCore;
            minUnsatCore = unsatCore;
            unsatCore = NULL;
        } else {
            cout << "critical " << test.getId() << endl;
            criticalLiterals.push_back(test);
        }
    }

    unsatCore = minUnsatCore;
...
Solver::shrinkUnsatCore()
{
    ...
    switch( wasp::Options::minimizationStrategy )
    {
        case MINIMIZATION_PROGRESSION:
            //cout << "c shrinkUnsatCore: progression" << endl;
            minimizeUnsatCoreWithProgression();
            return;

        case MINIMIZATION_LINEARSEARCH:
            //cout << "c shrinkUnsatCore: linearsearch" << endl;
